...
# extracted filters live here now
from src.motion.filters import HighPassFilter, LowPassFilter, SlewRateLimiter
...
class MotionAlgorithm:
    ...
    def __init__(self, config: dict):
        ...
        self.slew = SlewRateLimiter(self.max_delta)
    ...
    def calculate(self, telemetry: TelemetryData) -> Position6DOF:
        ...
        surge = (
            self.hp_surge.process(surge_in) * self.onset_gain
            + self.lp_surge.process(surge_in) * self.sustained_gain
        )

        sway = (
            self.hp_sway.process(sway_in) * self.onset_gain
            + self.lp_sway.process(sway_in) * self.sustained_gain
        )

        heave = (
            self.hp_heave.process(heave_in) * self.onset_gain
            + self.lp_heave.process(heave_in) * self.sustained_gain
        )

        # apply slew rate limiting
        surge = self.slew.limit(self._prev_surge, surge)
        sway = self.slew.limit(self._prev_sway, sway)
        heave = self.slew.limit(self._prev_heave, heave)
    ...
    def reset(self):
        """Reset filter state and previous output values."""
        self.hp_surge.reset()
        self.lp_surge.reset()
        ...
        with open(recording, "rb") as f:
            ...
            for _ in range(min(10, count)):
                ...
                if tel:
                    ...
                    print(
                        f"[{ts:.2f}s] "
                        f"G=[{tel.g_force_lateral:+.2f},{tel.g_force_longitudinal:+.2f},{tel.g_force_vertical:+.2f}] "
